...
const PercentComponent: React.FC<PercentProps> = ({
  initialWidth = 80, // 默认值
...
// 组件挂载时：维护运行时Map，初始化配置
  ...
  return (
    <div ref={componentRef} style={{ width: size.width, height: size.height }}>
      ...
      <Progress type="circle" percent={componentProps.percent} size={size.width} 
      strokeColor={componentProps.strokeColor} railColor={componentProps.railColor}
      strokeWidth={componentProps.strokeWidth} />
    </div>
  );
};

// 对外暴露：通过X6节点获取组件运行时实例（供外部调用）
export const getPercentComponentRuntime = (node: Node) => {
